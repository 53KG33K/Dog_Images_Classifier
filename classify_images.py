...
def classify_images(images_dir, results_dic, model):
    ...
    for key in results_dic:
        ...
        #Run classifier function to classify the image and return the classifier label
        model_label = classifier(image_path, model)

        # Process the classifier label to match the format of the pet label
        model_label = model_label.lower().strip()
        
        # Define the truth (pet label)
        truth = results_dic[key][0]

        # Compare pet label and classifier label and extend results_dic
